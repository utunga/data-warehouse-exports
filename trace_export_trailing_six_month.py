...
parser.add_argument('--export_file', default='trace_six_monthly.csv', 
...
# select trailing 6 month from trace
sql = """SELECT """ \
  ...
  WHERE
    convert(datetime, convert(nvarchar, [TRANSACTION_DATE_1])) > dateadd(month, -6, getdate())
